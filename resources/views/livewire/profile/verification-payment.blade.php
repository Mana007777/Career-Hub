...
<div
    ...
    @endif
>
    <div class="absolute top-0 inset-x-0 h-px bg-gradient-to-r from-transparent {{ $isPaid ? 'via-emerald-500/20' : 'via-cyan-500/20' }} to-transparent"></div>
    ...
    <div class="p-10 sm:p-14">
        <div class="flex flex-col xl:flex-row items-center justify-between gap-12">
            <div class="flex flex-col md:flex-row items-center gap-10 text-center md:text-left">
                <div class="relative shrink-0">
                    <div class="absolute inset-0 blur-[80px] opacity-10 group-hover:opacity-20 transition-opacity {{ $isPaid ? 'bg-emerald-500' : 'bg-cyan-500' }}"></div>
                    <div class="relative w-28 h-28 rounded-[2rem] bg-zinc-950 flex items-center justify-center border-2 shadow-2xl group-hover:scale-110 group-hover:rotate-6 transition-all duration-700 {{ $isPaid ? 'border-emerald-500/30' : 'border-cyan-500/30' }}">
                        <svg class="w-16 h-16 drop-shadow-[0_0_15px_rgba(34,211,238,0.5)] {{ $isPaid ? 'text-emerald-400' : 'text-cyan-400' }}" viewBox="0 0 24 24" fill="currentColor">
                ...
                <div class="space-y-4">
                    <h2 class="text-[10px] font-black uppercase tracking-[0.5em] italic {{ $isPaid ? 'text-emerald-500' : 'text-cyan-500' }}">Identity <span class="text-white">Verification</span></h2>
                    <h3 class="text-5xl font-black text-white tracking-tighter uppercase italic leading-none">{{ $isPaid ? 'Verification' : 'Blue' }} <span class="{{ $isPaid ? 'text-emerald-500' : 'text-cyan-500' }}">{{ $isPaid ? 'Complete' : 'Label' }}</span></h3>
                    <p class="text-zinc-500 text-lg max-w-sm leading-relaxed italic font-bold">
                        @if($isPaid)
                            Payment confirmed. Your verified badge is now active on your profile and posts.
                        @else
                            Complete payment to activate your verified badge across your profile and posts.
                        @endif
                    </p>
                </div>
            </div>
        ...
        @else
            <div class="mt-16 grid grid-cols-1 md:grid-cols-3 gap-8">
                <div class="bg-zinc-950/60 border border-zinc-800/80 rounded-[2.5rem] p-10 {{ $isPaid ? 'hover:border-emerald-500/20' : 'hover:border-cyan-500/20' }} transition-all duration-700 shadow-inner group/card">
                ...
                <div class="bg-zinc-950/60 border border-zinc-800/80 rounded-[2.5rem] p-10 {{ $isPaid ? 'hover:border-emerald-500/20' : 'hover:border-cyan-500/20' }} transition-all duration-700 shadow-inner group/card">
                    ...
                    <p class="mt-4 text-3xl font-black text-white italic tracking-tighter uppercase">{{ $verification?->type ?? 'Identity verification' }}</p>
                </div>
                <div class="bg-zinc-950/60 border border-zinc-800/80 rounded-[2.5rem] p-10 {{ $isPaid ? 'hover:border-emerald-500/20' : 'hover:border-cyan-500/20' }} transition-all duration-700 shadow-inner group/card">
            ...
            @if ($paymentLink)
                <div class="mt-12 p-12 bg-cyan-500/5 border border-cyan-500/20 rounded-[4rem] relative overflow-hidden">
                    ...
                    <div class="flex flex-col lg:flex-row items-center justify-between gap-10">
                        <div class="space-y-4">
                            <p class="text-[11px] font-black uppercase tracking-[0.4em] text-cyan-400 italic">Payment link ready</p>
                            @if ($readableCode)
            ...
            @if($isPaid)
                <div class="mt-16 p-8 bg-emerald-500/10 border border-emerald-500/20 rounded-[2.5rem] text-center">
                    <p class="text-emerald-300 text-[10px] font-black uppercase tracking-[0.35em]">
                        Verification is active. No further payment action is required.
                    </p>
                </div>
            @else
                <div class="mt-16 flex flex-wrap justify-center gap-8">
                    <button type="button"
                        ...
                        <span wire:loading.remove wire:target="startPayment">Start payment</span>
