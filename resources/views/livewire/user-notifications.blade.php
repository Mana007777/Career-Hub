...
>
    ...
    >
        <!-- Backdrop -->
        <div class="fixed inset-0 bg-black/90 backdrop-blur-md"></div>

        <!-- Modal -->
        <div 
            class="relative w-full max-w-2xl max-h-[90vh] mx-4 bg-black border border-white/5 shadow-2xl overflow-hidden rounded-[2.5rem]"
            @click.stop
            x-transition:enter="transition ease-out duration-300"
        ...
        >
            ...
            <div class="flex items-center justify-between p-6 border-b border-white/5">
                <div class="flex items-center gap-3">
                    <div class="p-2 bg-brand-deep/50 rounded-xl">
                        <svg class="w-6 h-6 text-brand-violet" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex items-center gap-2">
                    @if($this->unreadCount > 0)
                        <button 
                            ...
                            class="px-4 py-2 text-sm font-black dark:bg-brand-purple bg-indigo-600 text-white rounded-xl shadow-lg dark:shadow-brand-purple/20 transition-all uppercase tracking-widest text-[10px]"
                        >
                            Mark all as read
                        </button>
                    @endif
                ...
                @if($notifications->count() > 0)
                    <div class="divide-y divide-white/5">
                        @foreach($notifications as $notification)
                            <div 
                                class="p-4 hover:bg-white/5 transition-colors {{ !$notification->is_read ? 'bg-brand-deep/10 border-l-4 border-brand-purple' : '' }}"
                            >
                                    ...
                                    <div class="flex-shrink-0 mt-1">
                                        @switch($notification->type)
                                            ...
                                            @case('new_post_from_following')
                                                ...
                                                @break
                                            @case('suspension_user_expired')
                                            @case('suspension_post_expired')
                                            @case('post_suspended')
