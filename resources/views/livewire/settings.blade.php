    ...
    <div class="max-w-4xl mx-auto px-6 py-12" x-show="loaded" x-cloak>
        ...
        >
            <div class="flex items-center gap-6 mb-6">
                ...
                <h2 class="text-[10px] font-black text-emerald-500 uppercase tracking-[0.6em] italic">{{ __('Account Settings') }}</h2>
                <div class="h-px flex-1 bg-gradient-to-r from-transparent via-emerald-500/20 to-transparent"></div>
            </div>
            ...
            <p class="text-zinc-500 text-sm mt-6 uppercase tracking-[0.2em] italic font-medium">{{ __('Manage your profile and app preferences.') }}</p>
        </div>
        ...
        <div class="space-y-8">
            ...
            <div class="group relative bg-zinc-950/60 border border-zinc-800/50 rounded-[3rem] p-10 backdrop-blur-3xl hover:bg-emerald-500/[0.02] hover:border-emerald-500/30 transition-all duration-700 shadow-[0_30px_60px_rgba(0,0,0,0.3)]">
                ...
                <div class="flex flex-col md:flex-row items-center justify-between gap-10">
                    ...
                    <button wire:click="openProfileModal" class="w-full md:w-auto px-10 py-5 bg-emerald-500 text-black text-[10px] font-black uppercase tracking-[0.3em] rounded-2xl shadow-xl shadow-emerald-500/10 hover:shadow-emerald-500/20 hover:bg-emerald-400 transition-all italic">{{ __('Edit profile') }}</button>
                </div>
            </div>
            ...
            <div class="group relative bg-zinc-950/60 border border-zinc-800/50 rounded-[3rem] p-10 backdrop-blur-3xl hover:bg-rose-500/[0.02] hover:border-rose-500/30 transition-all duration-700 shadow-[0_30px_60px_rgba(0,0,0,0.3)]">
                ...
                <div class="flex flex-col md:flex-row items-center justify-between gap-10">
                    <div class="flex items-center gap-8">
                        ...
                        <div>
                            <h3 class="text-[10px] font-black text-zinc-600 uppercase tracking-[0.4em] italic mb-1">{{ __('Security') }}</h3>
                ...
                <div class="flex flex-col md:flex-row items-center justify-between gap-10">
                    <div class="flex items-center gap-8">
                        ...
                        <div>
                            ...
                            <p class="text-2xl font-black text-white uppercase tracking-tight italic">{{ __('Suspended Content & Users') }}</p>
                        </div>
                    </div>
                    <button wire:click="openSuspendedItemsModal" class="w-full md:w-auto px-10 py-5 bg-amber-500 text-black text-[10px] font-black uppercase tracking-[0.3em] rounded-2xl shadow-xl shadow-amber-500/10 hover:shadow-amber-500/20 hover:bg-amber-400 transition-all italic">{{ __('Manage Suspensions') }}</button>
                </div>
            </div>
            @endif
            ...
            <div class="group relative bg-zinc-950/60 border border-zinc-800/50 rounded-[3rem] p-10 backdrop-blur-3xl hover:bg-emerald-500/[0.02] hover:border-emerald-500/30 transition-all duration-700 shadow-[0_30px_60px_rgba(0,0,0,0.3)]">
                ...
                <div class="flex flex-col md:flex-row items-center justify-between gap-10">
                    <div class="flex items-center gap-8">
                        ...
                        <div>
                            <h3 class="text-[10px] font-black text-zinc-600 uppercase tracking-[0.4em] italic mb-1">{{ __('Account Info') }}</h3>
            ...
            <div class="group relative bg-zinc-950/60 border border-zinc-800/50 rounded-[3rem] p-10 backdrop-blur-3xl hover:bg-emerald-500/[0.02] transition-all duration-700">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12 pt-12 border-t border-zinc-800/50">
                    @foreach(['light' => [__('Light'), __('Light mode')], 'dark' => [__('Dark'), __('Dark mode')], 'system' => [__('Auto'), __('Use system setting')]] as $mode => $labels)
                        <label wire:click="setThemePreference('{{ $mode }}')" class="relative flex items-center gap-6 p-8 bg-zinc-950 border border-zinc-800 rounded-2xl transition-all cursor-pointer hover:border-emerald-500/30 group/mode {{ $themePreference === $mode ? 'border-emerald-500/50 ring-2 ring-emerald-500/10' : '' }}">
                            <div class="relative flex items-center justify-center">
                                <input type="radio" name="themePreference" wire:model.live="themePreference" value="{{ $mode }}" class="w-5 h-5 accent-emerald-500 cursor-pointer">
                            </div>
                            <div>
            ...
            <div class="group relative bg-zinc-950/60 border border-zinc-800/50 rounded-[3rem] p-10 backdrop-blur-3xl hover:bg-emerald-500/[0.02] transition-all duration-700" x-data="{ isOpen: false }">
                <div class="flex flex-col md:flex-row items-center justify-between gap-10">
                    <div class="flex items-center gap-8">
                        ...
                        <div>
                            <h3 class="text-[10px] font-black text-zinc-600 uppercase tracking-[0.4em] italic mb-1">{{ __('Language') }}</h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12 pt-12 border-t border-zinc-800/50" x-show="isOpen" x-transition:enter="transition cubic-bezier(0.16, 1, 0.3, 1) duration-500" x-transition:enter-start="opacity-0 -translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" style="display:none">
                    @foreach(['en' => [__('English'), __('English')], 'ar' => [__('Arabic'), __('Arabic')], 'ckb' => [__('Kurdish'), __('Kurdish')]] as $lang => $labels)
                        <label class="relative flex items-center gap-6 p-8 bg-zinc-950 border border-zinc-800 rounded-2xl hover:border-emerald-500/30 transition-all cursor-pointer group/mode {{ $locale === $lang ? 'border-emerald-500/50 ring-2 ring-emerald-500/10' : '' }}">
                            <div class="relative flex items-center justify-center">
                                <input type="radio" name="localePreference" wire:model="locale" wire:change="updateLocale" value="{{ $lang }}" class="w-5 h-5 accent-emerald-500 cursor-pointer">
                            </div>
                            <div>
                                <span class="block text-[8px] font-black text-zinc-600 uppercase tracking-widest group-hover/mode:text-emerald-500 transition-colors">{{ $labels[0] }}</span>
                                <span class="block text-sm font-black text-white uppercase tracking-tight italic">{{ $labels[1] }}</span>
                ...
                <div class="flex-1 overflow-y-auto p-10 custom-scrollbar bg-zinc-900/50">
                    <form wire:submit.prevent="updateProfile" class="space-y-10">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            @foreach(['name' => __('Name'), 'email' => __('Email'), 'username' => __('Username'), 'location' => __('Location'), 'website' => __('Website')] as $field => $label)
                            ...
                            <div class="col-span-full space-y-3">
                                <label for="bio" class="block text-[9px] font-black text-zinc-600 uppercase tracking-[0.4em] italic pl-4">{{ __('Bio') }}</label>
                        ...
                        <div class="flex items-center justify-end gap-6 pt-10 border-t border-zinc-800/50">
                            ...
                            <button type="submit" class="px-12 py-5 bg-emerald-500 text-black text-[10px] font-black uppercase tracking-[0.3em] rounded-2xl shadow-xl shadow-emerald-500/10 hover:bg-emerald-400 transition-all italic">{{ __('Save Changes') }}</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    @if($showReportsModal && auth()->check() && !auth()->user()->isAdmin())
        <div class="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-zinc-950/98 backdrop-blur-3xl" wire:click="closeReportsModal">
            <div class="bg-zinc-900 border border-zinc-800 rounded-[3rem] w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-[0_50px_100px_rgba(0,0,0,1)] relative" wire:click.stop>
                <div class="p-10 border-b border-zinc-800/50 bg-zinc-950/40 flex items-center justify-between">
                    <h2 class="text-[10px] font-black text-white uppercase tracking-[0.5em] italic italic">{{ __('Reports History') }}</h2>
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="bg-zinc-900/40 border border-zinc-800/50 rounded-2xl p-6 italic text-zinc-500 text-[11px] leading-relaxed font-bold font-medium selection:bg-emerald-500/20">
                                        @if($report->target_type === 'post' && $report->target)
                                            <p class="text-white mb-2 font-black italic">{{ __('Report') }}: {{ $report->target->title ?? __('Post') }}</p>
    ...
    @if($showSuspendedItemsModal && auth()->check() && auth()->user()->isAdmin())
        <div class="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-zinc-950/98 backdrop-blur-3xl" wire:click="closeSuspendedItemsModal">
            <div class="bg-zinc-900 border border-zinc-800 rounded-[3rem] w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col shadow-[0_50px_100px_rgba(0,0,0,1)]" wire:click.stop>
                <div class="p-10 border-b border-zinc-800/50 bg-zinc-950/40 flex items-center justify-between">
                    <h2 class="text-[10px] font-black text-white uppercase tracking-[0.5em] italic italic">{{ __('Suspensions Overview') }}</h2>
                ...
                <div class="flex-1 overflow-y-auto p-10 custom-scrollbar grid grid-cols-1 lg:grid-cols-2 gap-10 bg-zinc-900/50">
                    <!-- Suspended Users -->
                    <div class="space-y-6">
                        <h3 class="text-[10px] font-black text-zinc-600 uppercase tracking-[0.5em] italic pl-4 mb-4">{{ __('Suspended Users') }} ({{ $suspendedUsers->count() }})</h3>
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        @if($sUser->suspension)
                                             <div class="bg-amber-500/5 border border-amber-500/10 rounded-xl p-4 mb-4">
                                                 <p class="text-[9px] font-black text-amber-500/70 uppercase tracking-widest mb-2 italic">Reason: {{ $sUser->suspension->reason }}</p>
                    ...
                    <!-- Suspended Posts -->
                    <div class="space-y-6">
                        <h3 class="text-[10px] font-black text-zinc-600 uppercase tracking-[0.5em] italic pl-4 mb-4">{{ __('Suspended Posts') }} ({{ $suspendedPosts->count() }})</h3>
