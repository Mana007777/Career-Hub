    ...
    >
        ...
        <form wire:submit="{{ $submit }}">
            <div class="px-6 py-8 sm:px-10 sm:py-9">
                <div class="mx-auto w-full max-w-xl space-y-7 sm:max-w-2xl">
                    {{ $form }}
                </div>
            </div>
            ...
            @if (isset($actions))
                <div
                    class="flex flex-col-reverse items-stretch justify-end gap-3 border-t border-zinc-200 bg-zinc-50/95 px-6 py-4 sm:flex-row sm:items-center sm:justify-end sm:px-10 dark:border-zinc-800 dark:bg-zinc-950/95"
                >
                    {{ $actions }}
                </div>
            @endif
